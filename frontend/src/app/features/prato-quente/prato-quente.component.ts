...
// Categorias de PRATO QUENTE
const CATS_QUENTE = [
  'pratos','refeicoes','refeicao','massas','massa','pizzas','pizza',
  'carnes','carne','churrasco','grelhados','grelhado','assados','assado',
  'sopas','sopa','caldos','caldo','quente','quentes','lanches','lanche',
  'hamburguer','hamburgueres','porcoes','porcao','frituras','fritura',
  'fritos','sanduiches','sanduiche','wraps','tapiocas','tapioca'
];

@Component({ selector: 'app-prato-quente', standalone: true, imports: [CommonModule], templateUrl: './prato-quente.component.html' })
export class PratoQuenteComponent implements OnInit, OnDestroy {
  user: any;
  comandas: any[] = [];
  timer: any;
  ultimaAtualizacao = '';
  ...
  itens(c: any): any[] {
    if (!c.itens) return [];
    return c.itens.filter((it: any) => {
      const cat = this.normalize(it.produto?.categoria?.nome || '');
      return CATS_QUENTE.some(k => cat.includes(k));
    });
  }
  ...
  loadComandas() {
    forkJoin({ abertas: this.api.getComandasAbertas(), emPreparo: this.api.getComandasEmPreparo() }).subscribe({
      next: ({ abertas, emPreparo }) => {
        const todas = [...abertas, ...emPreparo]
          .filter(c => this.temItens(c))
  ...
  avancar(c: any) {
    ...
    if (next[c.status]) this.api.atualizarStatusComanda(c.id, next[c.status]).subscribe(() => this.loadComandas());
  }
  ...
  tempo(dt: string) {
    ...
    if (diff < 1) return 'agora'; if (diff === 1) return '1 min'; return diff + ' min';
  }
  tempoClass(dt: string) {
    const diff = Math.floor((Date.now() - new Date(dt).getTime()) / 60000);
    if (diff >= 15) return 'tempo-urgente'; if (diff >= 8) return 'tempo-alerta'; return 'tempo-ok';
  }
}
