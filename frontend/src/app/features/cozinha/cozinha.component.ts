  ...
  loadComandas() {
    forkJoin({ abertas: this.api.getComandasAbertas(), emPreparo: this.api.getComandasEmPreparo() }).subscribe({
      next: ({ abertas, emPreparo }) => {
        const todas = [...abertas, ...emPreparo].sort((a, b) => new Date(a.abertaEm).getTime() - new Date(b.abertaEm).getTime());
        if (todas.length > this.qtdAnterior && this.qtdAnterior > 0) { this.novaComanda = true; setTimeout(() => this.novaComanda = false, 3000); }
