...
// Categorias que pertencem à COPA
...
export class CopaComponent implements OnInit, OnDestroy {
  ...
  // Filtra apenas itens que pertencem à copa
  itensCopa(comanda: any): any[] {
  ...
  // Comanda tem itens para copa?
  temItensCopa(comanda: any): boolean {
    return this.itensCopa(comanda).length > 0;
  }
    ...
    forkJoin({
      ...
      next: ({ abertas, emPreparo }) => {
        const todas = [...abertas, ...emPreparo]
          .filter(c => this.temItensCopa(c))
          .sort((a, b) => new Date(a.abertaEm).getTime() - new Date(b.abertaEm).getTime());

        if (todas.length > this.qtdAnterior && this.qtdAnterior >= 0) {
          if (this.qtdAnterior > 0) {
            this.novaComanda = true;
            setTimeout(() => this.novaComanda = false, 4000);
          }
        }
